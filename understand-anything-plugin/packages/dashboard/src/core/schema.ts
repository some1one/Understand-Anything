...
// Companion embeddings file (embeddings.json). Validated on load; a malformed
// or absent file simply disables semantic search.
export const GraphEmbeddingsSchema = z.object({
  model: z.string(),
  dim: z.number(),
...
export function validateEmbeddings(
  data: unknown,
): z.infer<typeof GraphEmbeddingsSchema> | null {
  const result = GraphEmbeddingsSchema.safeParse(data);
